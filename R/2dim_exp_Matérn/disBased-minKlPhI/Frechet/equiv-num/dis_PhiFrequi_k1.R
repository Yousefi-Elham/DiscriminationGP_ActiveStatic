...
crit.fun.old <- function(theta,sigma2,des){
  ...
  if(n==1){
    ...
    if(det(kernel1)<=0 | det(kernel2)<=0){
      ...
      #eig.v <- eigen(kernel1)$values
      #v.mat <- eigen(kernel1)$vectors
      #lam.mat <- diag(eig.v)
      
      
      #kernel1.sqrt <- v.mat %*% lam.mat^(1/2) %*% t(v.mat)
      
      #B <- kernel1.sqrt %*% kernel2 %*% kernel1.sqrt
      #B.eigen <- eigen(B)$value
      
      #neg.count <- which(B.eigen < 0)
      #B.eigen[neg.count] <- 0
      
      #Frechet.crit <- sum(eigen(kernel1)$value) + sum(eigen(kernel2)$value) - 2*sum(sqrt(B.eigen))
      
      eigenvals <- Re(eigen(kernel1%*%kernel2, symmetric=FALSE, only.values = TRUE)$values)
      neg.count <- which(eigenvals < 0)
      eigenvals[neg.count] <- 0
      
      Frechet.crit <- sum(eigen(kernel1, only.values = TRUE)$value) + 
        sum(eigen(kernel2, only.values = TRUE)$value) -
        2*sum(sqrt(eigenvals))
    }
...
crit.fun <- function(theta,sigma2,des,cov.past.1,cov.past.2){
  ...
  if(det(kernel1.n)<=0 | det(kernel2.n)<=0){
    ...
    eigenvals <- Re(eigen(kernel1.n %*% kernel2.n, symmetric=FALSE, only.values = TRUE)$values)
    neg.count <- which(eigenvals < 0)
    eigenvals[neg.count] <- 0
    
    Frechet.crit <- sum(eigen(kernel1.n, only.values = TRUE)$value) + 
      sum(eigen(kernel2.n, only.values = TRUE)$value) -
      2*sum(sqrt(eigenvals))
...
system.time({
  ...
  for( k in 1: nleft){
    ...
    if (is.null(kernel1.n) || is.null(kernel2.n)) {
...
jpeg("G:/Other computers/My Computer/3files/2022/2-Feb/GP-project-R/Matern1.5-2.5-newpar/2dim_exp/disBased-minKlPhI/Frechet/equiv-num/design.jpeg")
...
#----------------------------------------------------------------
setEPS()
postscript("G:/Other computers/My Computer/3files/2022/2-Feb/GP-project-R/Matern1.5-2.5-newpar/2dim_exp/disBased-minKlPhI/Frechet/equiv-num/design.eps")
...
jpeg("G:/Other computers/My Computer/3files/2022/2-Feb/GP-project-R/Matern1.5-2.5-newpar/2dim_exp/disBased-minKlPhI/Frechet/equiv-num/des.seg.jpeg")
...
#----------------------------------------------------------------
setEPS()
postscript("G:/Other computers/My Computer/3files/2022/2-Feb/GP-project-R/Matern1.5-2.5-newpar/2dim_exp/disBased-minKlPhI/Frechet/equiv-num/des.seg.eps")
...
jpeg("G:/Other computers/My Computer/3files/2022/2-Feb/GP-project-R/Matern1.5-2.5-newpar/2dim_exp/disBased-minKlPhI/Frechet/equiv-num/des.seg1.jpeg", width=650, height=500)
...
#----------------------
setEPS()
pdf("G:/Other computers/My Computer/3files/2022/2-Feb/GP-project-R/Matern1.5-2.5-newpar/2dim_exp/disBased-minKlPhI/Frechet/equiv-num/des.seg1.eps", width = 11, height = 7)
...
jpeg("G:/Other computers/My Computer/3files/2022/2-Feb/GP-project-R/Matern1.5-2.5-newpar/2dim_exp/disBased-minKlPhI/Frechet/equiv-num/DesDis.jpeg")
...
#----------------------------------------------------------------
setEPS()
postscript("G:/Other computers/My Computer/3files/2022/2-Feb/GP-project-R/Matern1.5-2.5-newpar/2dim_exp/disBased-minKlPhI/Frechet/equiv-num/DesDis.eps")
...
#--------------------
setEPS()
postscript("G:/Other computers/My Computer/3files/2022/2-Feb/GP-project-R/Matern1.5-2.5-newpar/2dim_exp/disBased-minKlPhI/Frechet/equiv-num/DesDis1.eps", width = 11, height = 7)
...
jpeg("G:/Other computers/My Computer/3files/2022/2-Feb/GP-project-R/Matern1.5-2.5-newpar/2dim_exp/disBased-minKlPhI/Frechet/equiv-num/criterionval.jpeg")
...
ytick=seq(0,max(CRIT.vec),by=0.2)
axis(1,at=xtick,labels=T,cex.axis=1.5)
axis(2,at=ytick,labels=T,cex.axis=1.5)
box()
...
#----------------------------------------------------------------
setEPS()
postscript("G:/Other computers/My Computer/3files/2022/2-Feb/GP-project-R/Matern1.5-2.5-newpar/2dim_exp/disBased-minKlPhI/Frechet/equiv-num/criterionval.eps")
